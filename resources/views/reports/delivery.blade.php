...
@php
    $hasFilters = request('date_from') || request('location_id') || request('category_id') || request()->has('generate');
@endphp
...
@if($hasFilters)
    ...
    <div class="no-print mb-3 d-flex gap-2 justify-content-end">
        ...
        <a href="{{ route('reports.delivery.export', ['date_from' => request('date_from', date('Y-m-01')), 'date_to' => request('date_to', date('Y-m-d')), 'location_id' => request('location_id'), 'category_id' => request('category_id'), 'format' => 'pdf']) }}" class="btn btn-danger btn-sm">
            <i class="fas fa-file-pdf me-1"></i> PDF
        </a>
        <a href="{{ route('reports.delivery.export', ['date_from' => request('date_from', date('Y-m-01')), 'date_to' => request('date_to', date('Y-m-d')), 'location_id' => request('location_id'), 'category_id' => request('category_id'), 'format' => 'excel']) }}" class="btn btn-success btn-sm">
                    ...
                    <table class="table table-bordered table-sm report-table">
                        ...
                        <tbody>
                            ...
                            <tr class="category-row">
                                <td colspan="9"><strong>{{ strtoupper($category) }}</strong></td>
                            </tr>
                            @foreach($items as $d)
                            <tr>
                            ...
                            @endforeach
                            @empty
                            <tr><td colspan="9" class="text-center py-4 text-muted">No delivery records found</td></tr>
                            @endforelse
                        </tbody>
                    </table>
                </div>
...
@else
    <div class="alert alert-info no-print">
        ...
        <strong>Select filters and click "Generate" to preview the report.</strong>
    </div>
@endif
